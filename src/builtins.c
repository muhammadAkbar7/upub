...
builtin_cd(struct command *cmd, struct builtin_redir const *redir_list)
{
  ...
  // Debug print to verify target_dir
  // dprintf(get_pseudo_fd(redir_list, STDOUT_FILENO), "Debug: target_dir = %s\n", target_dir); 
  // chdir(target_dir); og code/26 and not updated runner.c
  if (chdir(target_dir) != 0) {
    // chdir failed; print an error message and return -1
    return -1;
  }
...
builtin_exit(struct command *cmd, struct builtin_redir const *redir_list)
{
  ...
  if (cmd->word_count > 1 && cmd->words[1] != NULL) {
    // Process the provided exit status argument
    char *string = cmd->words[1];
    char *end = NULL;
    long parsed_code = strtol(string, &end, 10);

    // Validate if the argument is a valid number
    if (*end != '\0' || errno == ERANGE) { // from https://man7.org/linux/man-pages/man3/errno.3.html
      // Non-numeric or out-of-range argument
      params.status = 255; // Default exit code for invalid argument
    } else {
      params.status = (int)parsed_code; // Update exit status with argument
    }
  }

  // If no argument is provided, retain the current exit status
  // (i.e., do not change params.status).

  bigshell_exit(); // Exit the shell process
  return -1; // This is unreachable but added for clarity
}
