...
err:
  ...
  free(cl);
    if (params.status == 0) { // Only set 128 if status wasn't explicitly set
      params.status = 128; // Generic error code for unexpected errors
  }
  //params.status = 128; // og 127 and had jus this code and no if statement
  warn(0);
  bigshell_exit();
}
