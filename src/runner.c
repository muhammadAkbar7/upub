...
expand_command_words(struct command *cmd)
{
  for (size_t i = 0; i < cmd->word_count; ++i) {
    expand(&cmd->words[i]);
    // fprintf(stderr, "Debug: Expanded word %zu: %s\n", i, cmd->words[i]);
  }
...
{ // put print statements to check assignments and add 
  for (size_t i = 0; i < cmd->assignment_count; ++i) {
    ...
    if (export_all) {
      vars_export(a->name); // vars_export marks the variable for export
    }
  }
  return 0;
}
...
if (move_fd(fd, r->io_number) < 0) {
    perror("move_fd");
    close(fd); // Clean up the file descriptor if move_fd fails
    status = -1;
    break;
}
...
run_command_list(struct command_list *cl)
{
  ...
  for (size_t i = 0; i < cl->command_count; ++i) {
     ...
     */
    // int const upstream_pipefd = -1; og code
     ...
     */
    // int const did_fork = 0; /* TODO */ og code
    ...
    if (child_pid == 0) {
      if (is_builtin) {
        ...
        // fprintf(stderr, "Debug: Running builtin: %s\n", cmd->words[0]);

        /* XXX Here's where we call the builtin function */
        int result = builtin(cmd, redir_list);

        // fprintf(stderr, "Debug: Builtin %s exited with status %d\n", cmd->words[0], result);
        ...
        params.status = result ? 129 : 0; // og is 127
        // fprintf(stderr, "Debug: Exit command setting status for runner %d\n", params.status);
         ...
         */
        if (has_upstream_pipe && move_fd(upstream_pipefd, STDIN_FILENO) < 0) {
          perror("move_fd");
          exit(1);
        }

        if (has_downstream_pipe && move_fd(downstream_pipefd, STDOUT_FILENO) < 0) {
          perror("move_fd");
          exit(1);
        }
         ...
         */
        //if (signal_restore() < 0) err(1, 0); this is the og one 
        // fprintf(stderr, "Debug: Restoring signals before execvp\n");
if (signal_restore() < 0) { // new one for testing purposes
    perror("signal_restore failed");
    err(1, 0);
}
         ...
         */
        //  fprintf(stderr, "Debug: About to execvp: %s\n", cmd->words[0]);
         if (execvp(cmd->words[0], cmd->words) < 0) {
          perror("execvp failed");
            err(123, 0); /* Exec failure -- why might this happen? */ //og is 127
        }

        err(121, 0); /* Exec failure -- why might this happen? */ // og is 127
     ...
     * a child process */
    assert(child_pid > 0);
    //     fprintf(stderr, "Debug: Process group ID: %d\n", getpgid(child_pid));
    // fprintf(stderr, "Debug: Parent shell process ID: %d\n", getpid());
    ...
    if (is_fg) {
      if (wait_on_fg_pgid(pipeline_data.pgid) < 0) {
        warn(0);
        // params.status = 120; // og is 127 ch
        return -1;
      }
    } else {
